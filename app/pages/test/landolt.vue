...
import { useLandoltTest, type LandoltDirection } from '~/composables/useLandoltTest'
...
// Botões de direção
const directionButtons: { direction: LandoltDirection; icon: string; label: string }[] = [
...
function handleKeyDown(event: KeyboardEvent) {
  ...
  if (phase.value === 'testing') {
    ...
    if (action === 'up' || action === 'down' || action === 'left' || action === 'right') {
      handleDirectionInput(action as LandoltDirection)
      return
    }
  ...
  <div class="test-page d-flex flex-column fill-height">
    ...
    <template v-if="phase === 'ready'">
      <header class="mb-6">
        ...
        <h1 class="text-h4 font-weight-bold">{{ $t('tests.landolt.title') }}</h1>
      </header>

      <v-card class="flex-grow-1 d-flex flex-column align-center justify-center pa-8">
        <div class="landolt-demo mb-6">
          <LandoltC :size="80" direction="right" />
        </div>
        ...
        <v-list class="bg-transparent mb-6" max-width="500">
          <v-list-item prepend-icon="mdi-numeric-1-circle">
            <v-list-item-title>{{ $t('tests.instructions.landolt.step1') }}</v-list-item-title>
          </v-list-item>
          <v-list-item prepend-icon="mdi-numeric-2-circle">
            <v-list-item-title>{{ $t('tests.instructions.landolt.step2') }}</v-list-item-title>
          </v-list-item>
          <v-list-item prepend-icon="mdi-numeric-3-circle">
            <v-list-item-title>{{ $t('tests.instructions.landolt.step3') }}</v-list-item-title>
          </v-list-item>
          <v-list-item prepend-icon="mdi-numeric-4-circle">
            <v-list-item-title>{{ $t('tests.instructions.landolt.step4', { distance: calibration.distanceM }) }}</v-list-item-title>
          </v-list-item>
        </v-list>
